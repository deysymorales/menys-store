...
class AjaxVentas
{
    ...
    public function AjaxGenerarTicketVenta($nro_boleta)
    {
        ...
        $pdf->Cell(60, 6, 'MeNys Store',0,0,'C');

        $pdf->Ln();
        $pdf->SetFont('Arial', '', 9);
        $pdf->Cell(60, 6, "Calle sobre el Arco",0,0,'C');  

        $pdf->Ln();
        $pdf->Cell(60, 6, utf8_decode("Chichicastenango"),0,0,'C');     
        ...
        foreach ($productos as $pro) {
            ...
            $pdf->Cell(25, 5,  "Q. " . number_format($pro["cantidad"] * $pro["precio_unitario_venta"], 2, ".", ","), 0, 0, "R");            
        ...
        $pdf->Cell(20, 4, "I.V.A:" );
        $pdf->Cell(40, 4, "Q. ". ROUND($total*0.12,2),0, 0, 'R');
        $pdf->Ln();

        $pdf->Cell(20, 4, "TOTAL A PAGAR: ");
        $pdf->Cell(40, 4, "Q. ". ROUND($total,2),0, 0, 'R');
        $pdf->Ln();
